...
- `Choice`: one selected option and a probability distribution over configured
  countries, `OTHER` and `UNKNOWN`. `OTHER` is a supported country outside the
...
configured country codes plus two abstention categories. The measured evaluation
...
addresses. Long receipts preserve their first/last 25 lines plus likely evidence lines.
