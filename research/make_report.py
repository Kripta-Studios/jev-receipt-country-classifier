...
def main():
    ...
    lines.extend(
        [
            ...
            "## Usage",
            "",
            f"Expanded experiment only; historical pilot excluded. There are {len(all_rows):,} prediction records: {len(live):,} successful uncached responses, {usage['cached_records']} cache replays and {usage['failed_records']} failed records. Extraction failures: {sum(r['status'] == 'extraction_error' for r in all_rows)}; provider failures: {sum(r['status'] == 'provider_error' for r in all_rows)}. Counts include both prompt variants. The estimated price uses the rate inspected on 2026-09-24; HTTP latency excludes OCR.",
            "",
            "```json",
